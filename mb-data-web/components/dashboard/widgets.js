'use client'
// components/dashboard/widgets.js — les widgets de « Vue d'ensemble ».
//
// Chaque widget est autonome et rendu par DashboardGrid à la position et à la
// largeur choisies par l'utilisateur. Ils partagent un seul calcul, fait une fois
// par useOverviewData() : recalculer la série mensuelle dans chaque widget
// referait le même parcours firms → accounts → payouts trois fois.
//
// Les graphes sont du SVG inline, pas Chart.js : ce sont de petits visuels, et le
// SVG résout `var(--…)` alors qu'un canvas ne le ferait pas — donc pas de
// reconstruction à chaque bascule de thème.
...
// Chemin SVG lissé passant par les points (Catmull-Rom simplifiée).
...
export function useOverviewData({ firms, accts, rates, toEUR, totalPayoutsEUR, totalSpentForAccount }) {
  ...
  // au bord du breach passe avant un payout à demander.
  const insight = useMemo(() => {
    ...
    const names = (list) => list.slice(0, 2).map(r => `${r.firm} ${r.a.name || ''}`.trim()).join(', ')
    if (atRisk.length) return {
      tone: 'risk', accent: 'var(--red)',
      title: t(atRisk.length > 1 ? 'app.hero.riskTitleN' : 'app.hero.riskTitle1').replace('{n}', atRisk.length),
      body: t('app.hero.riskBody').replace('{list}', names(atRisk)),
      cta: t('app.hero.riskCta'), firmId: atRisk[0].firmId,
    }
    if (eligible.length) return {
      tone: 'good', accent: 'var(--green)',
      title: t(eligible.length > 1 ? 'app.hero.payoutTitleN' : 'app.hero.payoutTitle1').replace('{n}', eligible.length),
      body: t('app.hero.payoutBody').replace('{list}', names(eligible)),
      cta: t('app.hero.payoutCta'), firmId: eligible[0].firmId,
    }
    return {
      tone: 'calm', accent: 'var(--blue)',
      title: t('app.hero.calmTitle').replace('{n}', accts.length),
      body: t('app.hero.calmBody').replace('{n}', firms.length),
      cta: null, firmId: null,
    }
  }, [firms, accts.length, t, totalPayoutsEUR, totalSpentForAccount])

  return { series, health, insight }
}
    ...
    <div style={cardOf(S)}>
      ...
      <svg viewBox="0 0 200 62" style={{ display: 'block', width: '100%', height: 'auto', marginTop: 16 }} aria-hidden="true">
        {vals.map((v, i) => {
          ...
          const h = Math.max(3, (v / max) * 56)
          return <rect key={i} x={i * w + 3} y={62 - h} width={w - 6} height={h} rx="4"
                       fill="var(--red)" opacity={0.35 + 0.55 * (v / max)} />
        })}
      </svg>
    </div>
  )
...
// Illustration du héros — décorative, donc masquée aux lecteurs d'écran.
