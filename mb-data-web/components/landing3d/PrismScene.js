'use client'
// PrismScene — 3D model adapted from Magic / 21st.dev "Ethereal".
// Cinematic faceted icosahedron with cosine-palette displacement shader,
// UnrealBloom + a cinematic post-processing pass (grain, CA, grading, vignette).
// Scroll/gsap coupling removed: gentle auto-rotation + mouse parallax instead.
...
export default function PrismScene({
  palette = { primary: '#6366f1', secondary: '#8b5cf6', tertiary: '#ec4899', accent: '#06ffa5' },
}) {
  ...
  const cinematicPostShader = {
    ...
    let scene, camera, renderer, composer, mesh, frameId
    ...
    cinePass.uniforms.uLetterbox.value = 1.5 // disable letterbox bars for full-bleed bg
    ...
    const animate = () => {
      ...
      mesh.material.uniforms.uMouse.value.set(mouse.sx, mouse.sy)
      mesh.rotation.y += 0.0016
      mesh.rotation.x = Math.sin(t * 0.18) * 0.18
      mesh.position.y = Math.sin(t * 0.45) * 0.06
      const last = composer.passes[composer.passes.length - 1]
      if (last && last.uniforms && last.uniforms.uTime) last.uniforms.uTime.value = t
      composer.render()
    }
    animate()
