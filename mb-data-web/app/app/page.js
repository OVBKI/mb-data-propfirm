...
export default function Home() {
  ...
  const [acctForm,setAcctForm]=useState({buyDate:'',currency:'USD',spent:'',activationFee:'',activationDate:'',status:'Challenge',notes:'',planSize:'50k',name:'',ddType:'static',payoutTarget:'',minTradingDays:'',minDailyProfit:'',profitSplit:'90',paymentMode:'monthly'})
  ...
  const [promoteForm,setPromoteForm]=useState({activationDate:'',activationFee:'',payoutTarget:'',minTradingDays:'',minDailyProfit:'',profitSplit:'90'})
  ...
  async function createFirm(){
    ...
    {const tg=defaultPayoutTarget(data.name,'50k');const md=defaultMinTradingDays(data.name,'50k');const pr=defaultChallengePrice(data.name,'50k');const mdp=defaultMinDailyProfit(data.name,'50k');const ps=suggestProfitSplit(data.name,'50k');setAcctForm({buyDate:new Date().toISOString().slice(0,10),currency:'USD',spent:pr!==null?String(pr):'',activationFee:'',activationDate:'',status:'Challenge',notes:'',planSize:'50k',name:'',ddType:defaultDdType(data.name),payoutTarget:tg!==null?String(tg):'',minTradingDays:md!==null?String(md):'',minDailyProfit:mdp!==null?String(mdp):'',profitSplit:String(ps),paymentMode:'monthly'})}
  }
