'use client'
// Page /app/groups — Gestion des groupes (Phase 3 réseau social, mai 2026).
//
// Fonctions :
//   - Créer un nouveau groupe (inline form)
//   - Rejoindre un groupe via code invitation
//   - Lister mes groupes (cards : name, members, role badge, [Voir])
//
// Layout : utilise AppSidebar comme les autres pages /app.
...
export default function GroupsPage() {
  ...
  // Load groups (whose I'm a member)
  useEffect(() => {
    if (!user) return
    loadGroups()
  }, [user])
  ...
  async function loadGroups() {
    ...
    // Récupère les groupes via group_members (RLS : on voit nos memberships)
    ...
    if (error) {
      ...
      // Enrichit avec role + filter null groups (orphans)
      const enriched = (memberships || [])
        .filter(m => m.groups)
        .map(m => ({ ...m.groups, role: m.role, joined_at: m.joined_at }))
      setGroups(enriched)
    }
  ...
  async function handleCreate(e) {
    ...
    if (name.length > 50) {
      setCreateError('Nom trop long (50 caractères max).')
      return
    }
    setCreating(true)
    try {
      // L'invite_code est généré par défaut SQL via generate_invite_code()
      ...
      setCreateDesc('')
      await loadGroups()
      // Redirige vers la page du groupe créé
  ...
  async function handleJoin(e) {
    ...
    if (!/^[A-Z0-9]{6}$/.test(code)) {
      setJoinError('Code invalide (6 caractères alphanumériques)')
      return
    }
    setJoining(true)
    try {
      ...
      setJoinSuccess(json.alreadyMember
        ? `Tu fais déjà partie de "${json.group_name}".`
        : `Tu as rejoint "${json.group_name}" ✓`)
      setJoinCode('')
      await loadGroups()
      // Redirige vers le groupe après 800ms
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, color: C.text, display: 'flex', fontFamily: 'inherit' }}>
      <AppSidebar
        user={user}
        currentPage="groups"
        currentHref="/app/groups"
      />
      ...
      <main style={{ flex: 1, overflow: 'auto' }}>
        <div style={{ maxWidth: 980, margin: '0 auto', padding: 32 }}>
          ...
          <div style={{
            ...
            <form onSubmit={handleCreate} style={cardStyle}>
              <div style={cardHeaderStyle}>
                <span style={{ fontSize: 18 }}>➕</span> Créer un groupe
              </div>
              <input
                type="text"
                placeholder="Nom du groupe (ex: Squad NQ Scalp)"
                value={createName}
            ...
            <form onSubmit={handleJoin} style={cardStyle}>
              <div style={cardHeaderStyle}>
                <span style={{ fontSize: 18 }}>🔑</span> Rejoindre via code
              </div>
              <input
                type="text"
                placeholder="ABCD12"
                ...
                required
              />
              <p style={{ fontSize: 11, color: C.text3, margin: '4px 0' }}>
                Le code à 6 caractères que ton pote t'a partagé.
              </p>
          ...
          <div>
            <h2 style={{
              fontSize: 11, color: C.text3, letterSpacing: '0.14em',
...
function GroupCard({ group }) {
  return (
    <Link href={`/g/${group.invite_code}`} style={{
          ...
          <div style={{
            ...
            {group.role === 'owner' && (
              <span style={{
                ...
                background: 'rgba(250,199,117,0.15)', color: C.amber, letterSpacing: '0.05em',
              }}>OWNER</span>
            )}
          </div>
          {group.description && (
