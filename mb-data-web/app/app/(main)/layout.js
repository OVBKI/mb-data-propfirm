...
import { PROPFIRM_RULES, FIRM_COLORS, MONTHS_FR, MONTHS_FULL, FIRM_SUGGESTIONS, FIRM_SUGGESTION_COLORS, STATUS_COLORS, PX_FIRMS, plansForFirm, accountLabel, defaultDdType, defaultPayoutTarget, defaultMinTradingDays, defaultChallengePrice, defaultMinDailyProfit, registerCustomFuturesFirms } from '../../../lib/constants'
...
import { suggestProfitSplit, generateAccountNames, buildAccountForm, programsForFirm } from '../../../lib/accountDefaults'
...
const cardS = { background: 'var(--card-bg)', border: 'var(--card-border)', borderRadius: 'var(--card-radius)', boxShadow: 'var(--card-shadow)' }
...
const S = {
  // La CARTE et le PANNEAU sont deux choses différentes. La carte vit dans le
  // flux de la page ; le panneau FLOTTE au-dessus (modale, tiroir) et ne peut
  // donc jamais être transparent. Ils partageaient le même style, ce qui rendait
  // impossible de rhabiller l'un sans casser l'autre.
  card: { background: 'var(--card-bg)', border: 'var(--card-border)', borderRadius: 'var(--card-radius)', boxShadow: 'var(--card-shadow)' },
  panel: { background: 'var(--panel-bg)', border: 'var(--panel-border)', borderRadius: 'var(--radius-lg)', boxShadow: 'var(--panel-shadow)' },
  input: { width: '100%', padding: '10px 12px', fontSize: '13px', border: '1px solid var(--hairline)', borderRadius: '8px', background: 'var(--tint1)', color: 'var(--text)', outline: 'none', transition: 'border-color 0.2s, background 0.2s', fontFamily: 'inherit' },
...
export default function AppLayout({ children }) {
  ...
  // L'assistant « nouveau compte » : null = fermé, sinon { firmId } (firmId null
  ...
  const [acctForm, setAcctForm] = useState({ buyDate: '', currency: 'USD', spent: '', activationFee: '', activationDate: '', status: 'Challenge', notes: '', planSize: '50k', program: '', name: '', ddType: 'static', payoutTarget: '', minTradingDays: '', minDailyProfit: '', profitSplit: '90', paymentMode: 'monthly', quantity: '1', customDrawdown: '' })
  ...
  async function createFirm() {
    ...
    { const tg = defaultPayoutTarget(data.name, '50k'); const md = defaultMinTradingDays(data.name, '50k'); const pr = defaultChallengePrice(data.name, '50k'); const mdp = defaultMinDailyProfit(data.name, '50k'); const ps = suggestProfitSplit(data.name, '50k'); setAcctForm({ buyDate: new Date().toISOString().slice(0, 10), currency: 'USD', spent: pr !== null ? String(pr) : '', activationFee: '', activationDate: '', status: 'Challenge', notes: '', planSize: '50k', program: '', name: '', ddType: defaultDdType(data.name), payoutTarget: tg !== null ? String(tg) : '', minTradingDays: md !== null ? String(md) : '', minDailyProfit: mdp !== null ? String(mdp) : '', profitSplit: String(ps), paymentMode: 'monthly', quantity: '1', customDrawdown: '' }) }
  }
  ...
  // La charge utile d'un compte à partir du formulaire. Partagée par le modal
  // classique et l'assistant : deux constructions séparées finiraient par
  // diverger sur un champ, et le compte créé dépendrait du chemin emprunté.
  // `market` suit le marché courant (les inserts CFD passent par CfdAccountModal).
  function accountPayload(firmId, form) {
    const isOneTime = form.paymentMode === 'onetime'
    return {
      firm_id: firmId, user_id: user.id,
      market: marketMode === 'cfd' ? 'cfd' : 'futures',
      buy_date: form.buyDate, currency: form.currency,
      spent: parseFloat(form.spent) || 0,
      activation_fee: isOneTime ? 0 : (parseFloat(form.activationFee) || 0),
      activation_date: form.activationDate || null,
      status: form.status, notes: form.notes,
      plan_size: form.planSize || '50k', dd_type: form.ddType || 'static',
      // Le PROGRAMME (Apex EOD / Intraday / Legacy, FundedNext Flex / Rapid…).
      // Vide → null : une chaîne vide ferait échouer les comparaisons côté lecture,
      // alors que null dit clairement « non renseigné » et fait retomber les
      // helpers sur le programme principal de la firme.
      program: form.program || null,
      payout_target: form.payoutTarget ? parseFloat(form.payoutTarget) : null,
      min_trading_days: form.minTradingDays ? parseInt(form.minTradingDays, 10) : null,
      min_daily_profit: form.minDailyProfit ? parseFloat(form.minDailyProfit) : null,
      profit_split: form.profitSplit ? parseInt(form.profitSplit, 10) : 90,
      payment_mode: form.paymentMode || 'monthly',
      custom_drawdown: form.customDrawdown ? parseFloat(form.customDrawdown) : null,
    }
  }
  ...
  async function saveAccount() {
    ...
    const basePayload = accountPayload(firmId, acctForm)
    let autoReset = false
  ...
  // L'assistant : une firme et un compte en une seule soumission. La firme n'est
  // créée que si elle n'existe pas déjà — choisir « Topstep » deux fois ne doit
  // pas produire deux Topstep.
  async function submitWizard({ firmName, existingFirmId, form }) {
    if (wizardBusy) return
    setWizardBusy(true)
    try {
      let firmId = existingFirmId
      if (!firmId) {
        const color = FIRM_COLORS[firms.length % FIRM_COLORS.length]
        const { data, error } = await supabase.from('firms')
          .insert({ name: firmName.trim(), color, user_id: user.id, market: marketMode === 'cfd' ? 'cfd' : 'futures' })
          .select().single()
        if (error || !data) {
          const quota = planLimitMessage(error, locale)
          showToast(quota || t('app.toasts.errorPrefix') + (error?.message || t('app.toasts.firmCreationFailed')))
      ...
      const basePayload = accountPayload(firmId, form)
      if (form.status === 'Financé') {
        basePayload.funded_date = form.activationDate || new Date().toISOString().slice(0, 10)
      }
      const names = quantity > 1
        ? generateAccountNames(form.name, quantity)
        : [(form.name || '').trim()]
      const { error } = await supabase.from('accounts').insert(names.map(n => ({ ...basePayload, name: n })))
      if (error) {
        const quota = planLimitMessage(error, locale)
        showToast(quota || t('app.toasts.errorPrefix') + (error.message || t('app.toasts.saveFailed')))
        return
      }

      setWizard(null)
      await loadFirms()
      showToast(quantity > 1 ? `${quantity} ${t('app.toasts.accountsCreatedSuffix')}` : t('app.toasts.accountAdded'))
      // On ouvre la firme : l'utilisateur voit tout de suite ce qu'il vient de
      // créer, au lieu de devoir le retrouver dans le dashboard.
      setFirmDrawer(firmId)
    } finally {
      setWizardBusy(false)
    }
  }
  ...
  async function submitQuickPayout({ acctId, date, amount, note }) {
    ...
    try {
      ...
      for (const f of firms) {
        ...
        if (a) { split = a.profit_split || suggestProfitSplit(f.name, a.plan_size, a.program || null) || 90; break }
      }
  ...
  async function savePayout() {
    ...
    const splitPct = acctForSave?.profit_split || suggestProfitSplit(firmForSave?.name, acctForSave?.plan_size, acctForSave?.program || null) || 90
  ...
  function openPromoteModal(firm, acct) {
    ...
    const ps = suggestProfitSplit(firm?.name, acct?.plan_size, acct?.program || null)
    setPromoteForm({
    ...
    <AppContext.Provider value={contextValue}>
      <div style={{ minHeight: '100vh', background: 'transparent', position: 'relative' }}>
        ...
        {wizard && (
          <NewAccountWizard
            firms={firms}
            customFirmNames={customFutures.map(f => f.name).filter(n => !FIRM_SUGGESTIONS.includes(n))}
            getFirmLogo={getFirmLogo}
            S={S}
            initialFirmId={wizard.firmId}
            busy={wizardBusy}
        ...
        {firmModal && (
          ...
          >
            <div
              ...
              style={{ ...S.panel, padding: '28px', width: '600px', maxWidth: '100%', maxHeight: '92vh', overflowY: 'auto', boxShadow: '0 24px 64px rgba(0,0,0,0.5)' }}
            >
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: '10px', marginBottom: '18px' }}>
                {[...FIRM_SUGGESTIONS, ...customFutures.map(f => f.name).filter(n => !FIRM_SUGGESTIONS.includes(n))].map(s => {
                  ...
                  const color = FIRM_SUGGESTION_COLORS[s] || 'var(--blue-light)'
                  return (
                    <button type="button" key={s} onClick={() => setNewFirmName(s)} title={s} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', padding: '14px 8px', borderRadius: '10px', background: isSelected ? 'var(--blue-bg)' : 'var(--surface2)', border: `1px solid ${isSelected ? 'var(--blue-light)' : 'var(--border2)'}`, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s', position: 'relative' }} onMouseEnter={e => { if (!isSelected) { e.currentTarget.style.borderColor = 'var(--blue-light)'; e.currentTarget.style.background = 'var(--surface3)' } }} onMouseLeave={e => { if (!isSelected) { e.currentTarget.style.borderColor = 'var(--border2)'; e.currentTarget.style.background = 'var(--surface2)' } }}>
                      ...
                      {isSelected && (<div style={{ position: 'absolute', top: 6, right: 6, width: 16, height: 16, borderRadius: 99, background: 'var(--blue-light)', color: 'var(--text-inverse)', fontSize: 10, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{'✓'}</div>)}
                    </button>
                  )
                })}
                {(() => {
                  ...
                  return (
                    <button type="button" onClick={() => { const input = document.getElementById('firm-custom-input'); if (input) input.focus() }} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '8px', padding: '14px 8px', borderRadius: '10px', background: isCustom ? 'var(--violet-bg)' : 'var(--surface2)', border: `1px dashed ${isCustom ? 'var(--violet)' : 'var(--border2)'}`, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s' }}>
                      <div style={{ width: 38, height: 38, borderRadius: 8, background: 'var(--violet-bg)', border: '1px solid var(--violet)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 18, color: 'var(--violet)', fontWeight: 700 }}>+</div>
              ...
              <div style={{ marginBottom: '20px' }}>
                <label style={S.label}>
                  ...
                  {newFirmName && !FIRM_SUGGESTIONS.includes(newFirmName) && !customFutures.some(f => f.name === newFirmName) && (
                    <span style={{ marginLeft: 8, fontSize: 9, padding: '2px 6px', borderRadius: 4, background: 'var(--violet-bg)', color: 'var(--violet)' }}>{t('app.firmModal.customBadge')}</span>
                  )}
                </label>
        ...
        {acctModal&&<div onClick={()=>setAcctModal(null)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.6)',zIndex:500,display:'flex',alignItems:'center',justifyContent:'center',padding:'12px',overflowY:'auto'}}><div ref={acctModalRef} role="dialog" aria-modal="true" tabIndex={-1} aria-label={t('app.acctModal.editTitle')} className="modal" onClick={e=>e.stopPropagation()} style={{...S.panel,padding:'28px',width:'440px',maxWidth:'100%',boxShadow:'0 24px 64px rgba(0,0,0,0.5)'}}><h3 style={{fontSize:'17px',fontWeight:'600',marginBottom:'20px'}}>{acctModal.acct?t('app.acctModal.editTitle'):`${t('app.acctModal.newTitlePrefix')} ${firms.find(f=>f.id===acctModal.firmId)?.name}`}</h3><div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'12px'}}><div><label style={S.label}>{t('app.acctModal.buyDate')}</label><input type="date" value={acctForm.buyDate} onChange={e=>setAcctForm(p=>({...p,buyDate:e.target.value}))} style={S.input} /></div><div><label style={S.label}>{t('app.acctModal.currency')}</label><select value={acctForm.currency} onChange={e=>setAcctForm(p=>({...p,currency:e.target.value}))} style={S.input}><option>USD</option><option>EUR</option><option>GBP</option><option>CHF</option></select></div><div><label style={S.label}>{t('app.acctModal.planSize')}</label><select value={acctForm.planSize} onChange={e=>{const newPlan=e.target.value;const firmName=firms.find(f=>f.id===acctModal.firmId)?.name;/* Le programme choisi peut ne pas exister a la nouvelle taille (Apex n'a que du legacy en 75K) : on le laisse tomber plutot que de garder un choix impossible. */const keep=programsForFirm(firmName,newPlan).includes(acctForm.program)?acctForm.program:'';const tg=defaultPayoutTarget(firmName,newPlan,keep||null);const md=defaultMinTradingDays(firmName,newPlan,keep||null);const pr=defaultChallengePrice(firmName,newPlan,keep||null);const mdp=defaultMinDailyProfit(firmName,newPlan,keep||null);const ps=suggestProfitSplit(firmName,newPlan,keep||null);setAcctForm(p=>({...p,planSize:newPlan,program:keep,payoutTarget:tg!==null?String(tg):p.payoutTarget,minTradingDays:md!==null?String(md):p.minTradingDays,spent:pr!==null?String(pr):p.spent,minDailyProfit:mdp!==null?String(mdp):p.minDailyProfit,profitSplit:String(ps)}))}} style={S.input}>{plansForFirm(firms.find(f=>f.id===acctModal.firmId)?.name).map(p=><option key={p} value={p}>{p.toUpperCase()}</option>)}</select></div>{(()=>{const firmName=firms.find(f=>f.id===acctModal.firmId)?.name;const list=programsForFirm(firmName,acctForm.planSize);/* Une seule offre : rien a choisir, le select serait du bruit. */if(list.length<2)return null;return(<div><label style={S.label}>{t('app.acctModal.program')}<TooltipIcon text={t('app.acctModal.programTip')} maxWidth={340} /></label><select value={acctForm.program} onChange={e=>{const pg=e.target.value;const tg=defaultPayoutTarget(firmName,acctForm.planSize,pg||null);const md=defaultMinTradingDays(firmName,acctForm.planSize,pg||null);const pr=defaultChallengePrice(firmName,acctForm.planSize,pg||null);const mdp=defaultMinDailyProfit(firmName,acctForm.planSize,pg||null);const ps=suggestProfitSplit(firmName,acctForm.planSize,pg||null);setAcctForm(p=>({...p,program:pg,payoutTarget:tg!==null?String(tg):p.payoutTarget,minTradingDays:md!==null?String(md):p.minTradingDays,spent:pr!==null?String(pr):p.spent,minDailyProfit:mdp!==null?String(mdp):p.minDailyProfit,profitSplit:String(ps)}))}} style={S.input}><option value="">{t('app.acctModal.programUnset')}</option>{list.map(pg=><option key={pg} value={pg}>{pg}</option>)}</select></div>)})()}<div><label style={S.label}>{t('app.acctModal.customDrawdown')}<TooltipIcon text={t('app.acctModal.customDrawdownTip')} maxWidth={320} /></label><input type="number" value={acctForm.customDrawdown} onChange={e=>setAcctForm(p=>({...p,customDrawdown:e.target.value}))} placeholder={t('app.acctModal.customDrawdownPlaceholder')} style={S.input} /></div><div style={{gridColumn:'1/-1'}}><label style={S.label}>{t('app.acctModal.paymentMode')}<TooltipIcon text={t('app.acctModal.paymentModeTip')} maxWidth={360} /></label><div style={{display:'flex',gap:'4px',background:'var(--surface3)',borderRadius:'var(--radius)',padding:'4px'}}>{[{v:'monthly',l:t('app.acctModal.monthly'),d:t('app.acctModal.monthlySub')},{v:'onetime',l:t('app.acctModal.onetime'),d:t('app.acctModal.onetimeSub')}].map(opt=>(<button key={opt.v} type="button" onClick={()=>setAcctForm(p=>({...p,paymentMode:opt.v,activationFee:opt.v==='onetime'?'':p.activationFee}))} style={{flex:1,padding:'10px 12px',fontSize:'12px',fontWeight:'600',background:acctForm.paymentMode===opt.v?'var(--blue)':'transparent',color:acctForm.paymentMode===opt.v?'#fff':'var(--text2)',border:'none',borderRadius:'6px',cursor:'pointer',fontFamily:'inherit',transition:'all 0.15s',display:'flex',flexDirection:'column',alignItems:'center',gap:'2px'}}><span>{opt.l}</span><span style={{fontSize:'10px',opacity:0.75,fontWeight:'500'}}>{opt.d}</span></button>))}</div></div><div><label style={S.label}>{acctForm.paymentMode==='onetime'?t('app.acctModal.oneTimePriceLabel'):t('app.acctModal.monthlyPriceLabel')}<TooltipIcon text={acctForm.paymentMode==='onetime'?t('app.acctModal.oneTimePriceTip'):t('app.acctModal.monthlyPriceTip')} maxWidth={320} /></label><input type="number" value={acctForm.spent} onChange={e=>setAcctForm(p=>({...p,spent:e.target.value}))} placeholder={t('app.acctModal.pricePlaceholder')} style={S.input} /></div><div style={{gridColumn:'1/-1'}}><label style={S.label}>{t('app.acctModal.status')}</label><select value={acctForm.status} onChange={e=>setAcctForm(p=>({...p,status:e.target.value}))} style={S.input}><option>Challenge</option><option>Financé</option><option>Échoué</option></select></div>{acctForm.status==='Financé'&&<div style={{gridColumn:'1/-1',background:'var(--green-bg)',border:'0.5px solid var(--green)',borderRadius:'var(--radius)',padding:'12px'}}><div style={{fontSize:'11px',fontWeight:'700',color:'var(--green-text)',marginBottom:'10px',textTransform:'uppercase',letterSpacing:'0.5px'}}>{'✅'} {t('app.acctModal.fundedSection')}</div><div style={{display:'grid',gridTemplateColumns:acctForm.paymentMode==='onetime'?'1fr':'1fr 1fr',gap:'8px'}}><div><label style={S.label}>{t('app.acctModal.activationDate')}</label><input type="date" value={acctForm.activationDate} onChange={e=>setAcctForm(p=>({...p,activationDate:e.target.value}))} style={{...S.input,background:'var(--surface3)'}} /></div>{acctForm.paymentMode!=='onetime'&&<div><label style={S.label}>{t('app.acctModal.activationFee')}</label><input type="number" value={acctForm.activationFee} onChange={e=>setAcctForm(p=>({...p,activationFee:e.target.value}))} placeholder={t('app.acctModal.activationFeePlaceholder')} style={{...S.input,background:'var(--surface3)'}} /></div>}</div>{acctForm.paymentMode==='onetime'&&<div style={{marginTop:'8px',fontSize:'11px',color:'var(--text3)',display:'flex',gap:'6px',alignItems:'center'}}>{'💎'} {t('app.acctModal.onetimeNote')}</div>}</div>}<div style={{gridColumn:'1/-1'}}>{!acctModal.acct ? (
                  <>
                    ...
                    {(() => {
                      ...
                      return (
                        <div style={{marginTop:'8px',padding:'10px 12px',background:'var(--blue-bg)',border:'0.5px solid var(--blue-border)',borderRadius:'var(--radius)',fontSize:'11px',color:'var(--text2)',lineHeight:1.55}}>
                          ...
                          {totalPrice > 0 && (
                            <div style={{color:'var(--text3)',marginTop:'2px'}}>
                              {acctForm.paymentMode === 'onetime' ? t('app.acctModal.bulkOneTime') : t('app.acctModal.bulkMonth1')} {'·'} {t('app.acctModal.total')} <strong style={{color:'var(--red)'}}>{totalPrice.toFixed(2)} {currencySymbol}</strong> ({qty} {'×'} {acctForm.spent||0} {currencySymbol})
                            </div>
                          )}
                        </div>
                      )
        ...
        {promoteModal && (() => {
          ...
          return (
            <div onClick={()=>setPromoteModal(null)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.75)',backdropFilter:'blur(4px)',zIndex:600,display:'flex',alignItems:'center',justifyContent:'center',padding:'16px',overflowY:'auto'}}>
              <div ref={promoteModalRef} role="dialog" aria-modal="true" tabIndex={-1} aria-label={t('app.fundedModal.congrats')} onClick={e=>e.stopPropagation()} style={{...S.panel,padding:'28px',width:'500px',maxWidth:'100%',boxShadow:'0 30px 80px rgba(0,0,0,0.6)',position:'relative',overflow:'hidden'}}>
                <div style={{position:'absolute',inset:0,opacity:0.6,background:'radial-gradient(ellipse 80% 50% at 50% 0%, var(--green), transparent 60%)',pointerEvents:'none'}} />
                <div style={{position:'relative'}}>
                  ...
                  {isOneTime && (
                    <div style={{padding:'8px 12px',marginBottom:'12px',background:'var(--blue-bg)',border:'0.5px solid var(--blue-border)',borderRadius:'var(--radius)',fontSize:11,color:'var(--text3)'}}>
                  ...
                  <div style={{padding:'10px 12px',marginBottom:'18px',background:'var(--green-bg)',border:'0.5px solid var(--green)',borderRadius:'var(--radius)',fontSize:12,color:'var(--text2)',lineHeight:1.55}}>
                  ...
                  <div style={{display:'flex',gap:'8px',justifyContent:'flex-end'}}>
                    ...
                    <button onClick={savePromote} style={{padding:'11px 22px',fontSize:13,fontWeight:700,background:'linear-gradient(135deg, var(--green) 0%, #2ed694 100%)',border:'none',color:'#fff',borderRadius:'var(--radius)',cursor:'pointer',fontFamily:'inherit',boxShadow:'0 6px 18px var(--green-bg)',display:'flex',alignItems:'center',gap:'8px'}}>
        ...
        {failModal && (() => {
          ...
          return (
            <div onClick={()=>setFailModal(null)} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.78)',backdropFilter:'blur(4px)',zIndex:600,display:'flex',alignItems:'center',justifyContent:'center',padding:'16px',overflowY:'auto'}}>
              <div ref={failModalRef} role="dialog" aria-modal="true" tabIndex={-1} aria-label={t('app.failModal.challengeFailed')} onClick={e=>e.stopPropagation()} style={{...S.panel,padding:'28px',width:'460px',maxWidth:'100%',boxShadow:'0 30px 80px rgba(0,0,0,0.6)',position:'relative',overflow:'hidden'}}>
                <div style={{position:'absolute',inset:0,opacity:0.5,background:'radial-gradient(ellipse 80% 50% at 50% 0%, var(--red-bg), transparent 60%)',pointerEvents:'none'}} />
                <div style={{position:'relative'}}>
                  ...
                  <div style={{ padding:'16px 18px', marginBottom:'18px', background:'var(--surface2)', border:'1px solid var(--blue-border)', borderLeft:'3px solid var(--blue-light)', borderRadius:'var(--radius)', fontSize:13, color:'var(--text)', lineHeight:1.65, fontStyle:'italic' }}>
                    {failModal.message}
                  </div>
                  <div style={{padding:'8px 12px',marginBottom:'18px',background:'var(--amber-bg)',border:'0.5px solid var(--amber)',borderRadius:'var(--radius)',fontSize:11,color:'var(--text3)',lineHeight:1.5}}>
                  ...
                  <div style={{display:'flex',gap:'8px',justifyContent:'flex-end'}}>
                    ...
                    <button onClick={confirmFail} style={{padding:'10px 18px',fontSize:12,fontWeight:600,background:'var(--red-bg)',color:'var(--red)',border:'1px solid #e8504a',borderRadius:'var(--radius)',cursor:'pointer',fontFamily:'inherit',display:'flex',alignItems:'center',gap:'6px'}}>
        ...
        <CfdAccountModal
          open={cfdAddOpen || !!cfdEditAccount}
          account={cfdEditAccount}
          onClose={() => { setCfdAddOpen(false); setCfdEditAccount(null) }}
          onSaved={loadFirms}
          user={user}
          showToast={showToast}
        />

        {/* Open-beta feedback: dismissible banner + floating button + modal */}
        <BetaFeedback user={user} showToast={showToast} />
